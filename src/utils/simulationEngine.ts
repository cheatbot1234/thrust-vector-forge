...
export const runLiteSimulation = async (parameters: RocketParameters): Promise<SimulationResult> => {
  ...
  const throatArea = Math.PI * Math.pow(parameters.throatDiameter / 2000, 2); // mm² to m²
  const exitArea = throatArea * parameters.nozzleExpansionRatio;
  
  // Calculate chamber temperature based on mixture ratio and chamber pressure
  // This is a simplified model - in reality this would use CEA or other simulation
  const chamberTemperature = 1000 + 
    (parameters.chamberPressure * 200) + 
    (parameters.mixtureRatio * 500) - 
    (Math.pow(parameters.mixtureRatio - 2.1, 2) * 600);
  ...
  const exitPressure = chamberPressure * (1 / parameters.nozzleExpansionRatio) ** (gamma / (gamma - 1));
  ...
  const thrustCoefficient = Math.sqrt(((2 * gamma ** 2) / (gamma - 1)) * ((2 / (gamma + 1)) ** ((gamma + 1) / (gamma - 1))) * 
    (1 - pressureRatio ** ((gamma - 1) / gamma))) + parameters.nozzleExpansionRatio * (exitPressure - ambientPressure) / chamberPressure;
  
  // Calculate mass flow rate
  const massFlowRate = chamberPressure * throatArea * Math.sqrt((gamma / (R * chamberTemperature)) * 
    ((2 / (gamma + 1)) ** ((gamma + 1) / (gamma - 1))) * M);
  ...
  const engineLength = (parameters.chamberLength + parameters.throatDiameter * parameters.nozzleExpansionRatio / 2) / 1000; // m
  const numPoints = 50;
  ...
  const throatPosition = parameters.chamberLength / 1000;
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    if (x < throatPosition) {
      ...
      const effectiveExpansion = 1 + nozzleRatio * (parameters.nozzleExpansionRatio - 1);
